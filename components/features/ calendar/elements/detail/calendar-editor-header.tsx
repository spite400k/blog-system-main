...
import { saveCalendar } from '../../utils/save'
import { deleteCalendar } from '../../utils/delete'
...
export const CalendarEditorHeader = (props: { calendar: CalendarType }) => {
    ...
    <HeaderBox background={theme.color.base}>
      ...
      >
        ...
        <FlexBox way={'row'} gap={'1em'}>
          <HeaderButtonBox
            onClick={async () => {
              const result = await deleteCalendar(props.calendar)
              if (result) {
                await refresh(props.calendar)
          ...
          <Button
            onClick={async () => {
              ...
              if (error === null) {
                notifier.show(
                  props.calendar.publish
                    ? props.calendar.releaseDate.toDate() < new Date()
                      ? messageList.success_save_update
                      : messageList.success_save_reserve
                    : messageList.success_save_draft
                )
                await schedule(props.calendar)
              } else {
