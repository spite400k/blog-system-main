...
export const GameMovieEditorSidebar = (props: {
  gameMovie: GameMovie
  categories: Category[]
  isPreview: boolean
  ...
  const [isPublish, setPublish] = useState(props.gameMovie.publish)
  const [isCalendarVisible, setCalendarVisible] = useState(false)
  const [thumbnail, setThumbnail] = useState<StorageObject | null>(
    props.gameMovie.thumbnail ?? null
  )
  const [releaseDate, setReleaseDate] = useState<Date>(
    props.gameMovie.releaseDate
      ? props.gameMovie.releaseDate.toDate()
      : new Date()
  )
  const editor = useGameMovieEditor()

  useEffect(() => {
    props.gameMovie.publish = isPublish
  }, [isPublish, props.gameMovie])

  useEffect(() => {
    props.gameMovie.releaseDate = Timestamp.fromDate(releaseDate)
  }, [releaseDate, props.gameMovie])

  useEffect(() => {
    if (thumbnail) props.gameMovie.thumbnail = thumbnail
  }, [thumbnail, props.gameMovie])
        ...
        >
          ...
          >
            <Input
              ...
              background={theme.color.gray06}
              border={{ radius: '6px' }}
              defaultValue={props.gameMovie.slug}
              onChange={(e) => (props.gameMovie.slug = e.target.value)}
            />
          </BottomField>
          <BottomField
            title={'カテゴリー'}
              ...
              <Select
                ...
                defaultValue={
                  props.categories.filter(
                    (c) => c.id === props.gameMovie.category
                  ).length > 0
                    ? props.categories.filter(
                        (c) => c.id === props.gameMovie.category
                      )[0].name
                ...
                onChange={(s) => {
                  ...
                  props.gameMovie.category = category.id
                }}
              />
            )}
          </BottomField>
          ...
          >
            <Input
              ...
              border={{ radius: '6px' }}
              defaultValue={
                props.gameMovie.custom ? props.gameMovie.custom.color ?? '' : ''
              }
              onChange={(e) => {
                if (!props.gameMovie.custom) props.gameMovie.custom = {}
                props.gameMovie.custom.color = e.target.value
              }}
            />
          </BottomField>
        </FlexBox>
