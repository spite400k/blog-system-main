...
import { deleteGameMovie2air } from '../utils/delete'
...
export const GameMovie2airEditorHeader = (props: { gameMovie2air: GameMovie2air }) => {
    ...
    <HeaderBox background={theme.color.base}>
      ...
      >
        ...
        <FlexBox way={'row'} gap={'1em'}>
          <HeaderButtonBox
            onClick={async () => {
              const result = await deleteGameMovie2air(props.gameMovie2air)
              if (result) {
                await refresh(props.gameMovie2air)
                router.push('/gameMovie2air')
              }
            }}
          >
            <Image
          ...
          <Button
            onClick={async () => {
              const error = await saveGameMovie2air(props.gameMovie2air)
              if (error === null) {
                notifier.show(
                  props.gameMovie2air.publish
                    ? props.gameMovie2air.releaseDate.toDate() < new Date()
                      ? messageList.success_save_update
                      : messageList.success_save_reserve
                    : messageList.success_save_draft
                )
                await schedule(props.gameMovie2air)
              } else {
